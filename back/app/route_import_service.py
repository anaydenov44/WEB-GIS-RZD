...
def find_station_by_exact_code(connection, station_code_rzd: str | None):
    ...
    station_code_rzd = station_code_rzd.strip()
    if not station_code_rzd:
        return None
...
def find_station_by_exact_visible_name(connection, station_name_raw: str | None):
    ...
    rows = connection.execute(
        text("""
            ...
            ORDER BY is_main_rail_station DESC, id
            LIMIT 2;
        """),
    ...
    if len(rows) == 1:
        return {
            ...
            "match_method": "exact_visible_name",
...
def resolve_station_match(
    ...
    *,
    ...
    if allow_name_match:
        matched_by_name = find_station_by_exact_visible_name(connection, station_name_raw)
        if matched_by_name:
...
def prepare_route_fields(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(stops) < 2:
        raise ValueError("Маршрут должен содержать минимум 2 остановки")

    sorted_stops = sorted(stops, key=lambda item: item["stop_sequence"])
